...
class ModelVisitor extends SimpleElementVisitor<void> {
  ...
  void visitMethodElement(MethodElement element) {
    ...
    methods[element.name] = {"url": repoUrl, "cache": ""};

    try {
      switch (MedatadaExtractor.getMethodType(element)) {
        case "get":
          methods[element.name]!["cache"] +=
              (MedatadaExtractor.getFromElement(element, Get, 'cache') ?? "");
          break;
        case "getLocalStorage":
          methods[element.name]!["cache"] +=
              (MedatadaExtractor.getFromElement(
                    element,
                    GetLocalStorage,
                    'cache',
                  ) ??
        ...
        case "post":
          methods[element.name]!["cache"] +=
              (MedatadaExtractor.getFromElement(element, Post, 'cache') ?? "");
          break;
      }
    } catch (e) {}

    print(
      "-------->>>>>>> annotation metadata: ${MedatadaExtractor.getMethodType(element)} : cache: ${methods[element.name]!["cache"]}",
    );
    ...
    MedatadaExtractor.getMethodAnnotations(element, Headers).forEach((el) {
    ...
    try {
      element.parameters.forEach((paramElement) {
        ...
        if (element.metadata.firstOrNull.toString().contains("@Post")) {
          if (paramElement.metadata.firstOrNull.toString().contains(
            "@PostRequestModel",
          )) {
